...
- [DataLayer RPC API](https://docs.chia.net/docs/12rpcs/datalayer_rpcs 'DataLayer RPC API')
- [DataLayer CLI Reference](https://docs.chia.net/docs/13cli/datalayer_cli 'DataLayer CLI Reference')
...
### Install and configure Chia and the DataLayer
...
12. Configure your router to forward port 8575 (data propagation server) to your local machine. To configure your router's settings, typically you will need enter `http://192.168.1.1` in a web browser, though this address varies for different routers. From your router's settings, locate the Port Forwarding section and add a rule to forward port 8575 to your local IP address.

13. You will also need to configure your computer's firewall to allow connections on port 8575. This process is different for each OS:
...
Before the transaction has been confirmed, `get_root` will return an empty hash:
...
After the transaction has been confirmed, `get_root` will return a real hash:
...
- `a` -- The amount (in mojos) to spend to create the mirror. In theory, mirrors with a higher `amount` will be prioritized. Minimum `amount` is 0
